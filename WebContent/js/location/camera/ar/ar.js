var AugmentedRealityViewer = function(getPOI, options) {
	...
	this.setViewer = function(element) {
		...
		// container
		// .appendChild("<img src='../../images/icons/map-markers/building.png'
		// >");
	...
	function projectedPOI(here, obj) {
		...
		var dNorth = (here.latitude > obj.latitude ? -1 : 1)
				* distance(here, objNorth);
		var angle = (360 + (here.longitude > obj.longitude ? -1 : 1)
				* Math.acos(dNorth / d) * 180 / Math.PI) % 360;
		return {
			distance : d,
			angle : angle,
			label : obj.label
	...
	function setPOIy() {
		if (here && self.viewer) {
			for ( var i = 0; i < self.poi.length; i++) {
				...
				self.poi[i].y = overlay.height
						- Math.log(self.poi[i].distance)
						/ Math.log(Math.pow(maxDistance,
								1 / (overlay.height - 15)));
			}
			drawPOIInfo();
		}
		...
		if (prevAlpha === null || Math.abs(alpha - prevAlpha) > 1) {
			...
			for ( var i = 0; i < self.poi.length; i++) {
				...
				var x = overlay.width / 2
						+ ((360 + alpha - self.poi[i].angle) % 360) * 16 / 9;
...
//				var y = $(window).height() / 2;
				...
				ctx.lineTo(x + ($("#marker").width() / 2) - 7, y + 33
						+ $("#marker").height());
				ctx.stroke();
				ctx.font = "10px Arial";
				ctx.fillText(
						Math.floor(self.poi[i].distance / 100) / 10 + 'km',
						(overlay.width / 2) + (x - overlay.width) / 2,
						overlay.height + (y - overlay.height) / 2);
				ctx.font = "25px Arial";
	...
	// if options.remote is set, we don't try to capture the stream, orientation
	// and geolocation — they'll be set externally
